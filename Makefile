...
build-senyatyl-dongle: docker-image
	$(DOCKER_RUN) make _build-senyatyl-dongle

build-senyatyl-dongle-debug: docker-image
	$(DOCKER_RUN) make _build-senyatyl-dongle-debug
...
_build-senyatyl-dongle: west-init
	west build -p always -s zmk/app -d build/senyatyl_dongle -b seeeduino_xiao_ble -- -DZMK_CONFIG=$(ZMK_CONFIG) -DSHIELD="senyatyl_dongle prospector_adapter"

_build-senyatyl-dongle-debug: west-init
	west build -p always -s zmk/app -d build/senyatyl_dongle_debug -b seeeduino_xiao_ble -- -DZMK_CONFIG=$(ZMK_CONFIG) -DSHIELD="senyatyl_dongle prospector_adapter" -DSNIPPET=zmk-usb-logging -DCONFIG_LOG_BUFFER_SIZE=32768

_build-senyatyl-all: _build-senyatyl-left _build-senyatyl-right _build-senyatyl-right-peripheral _build-senyatyl-right-nomouse _build-senyatyl-right-col2row _build-senyatyl-dongle
